...
class FlashInferAttnBackend(AttentionBackend):
    ...
    def __init__(self, model_runner: ModelRunner):
        ...
        # KV index start/end pointers for each req
        self.kv_indptr = [
    ...
    def forward_extend(
        ...
        # init_forward_metadata sorts indices, then cache_loc is re-stored
        # to point to the correct KV cache locations.
        # The kernel computes using the info prepared during init forward,
        # which runs once per iteration.
...
class FlashInferIndicesUpdaterDecode:
    ...
    def call_begin_forward(
        ...
        # req_to_token has the index mapping, but it is not stored contiguously per batch.
        # Re-store contiguously using a triton kernel to reduce kernel access overhead.
...
def create_flashinfer_kv_indices_triton(
    ...
    kv_start = 0
    kv_end = 0
    # kv_start is computed this way if provided; usually receives None.
    ...
    # req_to_token_pool.req_to_token maps each req to its token cache locations.
    # Load it into a new kv_indices torch.empty to optimize GPU global memory
