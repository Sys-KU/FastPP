...
class CudaGraphRunner:
    ...
    def __init__(self, model_runner: "ModelRunner"):
        ...
        self.pp_size = self.model_runner.pp_size
        self.hidden_size = self.model_runner.model_config.hidden_size
        ...
        if self.model_runner.server_args.attention_backend == "flashinfer":
        ...
        with torch.device("cuda"):
            ...
            self.intermediate_tensors = torch.zeros((self.max_bs << 1, self.hidden_size),
    ...
    def capture_one_batch_size(self, bs: int, forward: Callable):
        ...
        if self.model_runner.pp_group.is_first_rank:
            intermediate_tensors = None
        else:
            intermediate_tensors = self.intermediate_tensors[:bs << 1]
        ...
        if self.model_runner.server_args.attention_backend == "flashinfer":
            ...
            self.model_runner.attn_backend.init_forward_metadata_capture_cuda_graph(
                ...
                req_pool_indices,
                seq_lens,
                self.model_runner._req_to_token_pool.req_to_token,
                encoder_lens,
            )
    ...
    def replay(self, forward_batch: ForwardBatch, intermediate_tensors: Optional[torch.Tensor] = None):
        ...
        if not self.model_runner.pp_group.is_first_rank:
            self.intermediate_tensors[:raw_bs << 1].copy_(intermediate_tensors)
        if self.is_encoder_decoder:
        ...
        if not self.model_runner.pp_group.is_last_rank:
            intermediate_tensors = self.output_buffers[bs][:raw_bs << 1]
            return intermediate_tensors
        else:
