...
def launch_engine(
    ...
    if server_args.dp_size == 1:
        ...
        pp_size_per_node = server_args.pp_size // server_args.nnodes
        pp_rank_range = range(
        ...
        for pp_rank in pp_rank_range:
            for tp_rank in tp_rank_range:
                ...
                # gpu_id = local_rank, used for torch.cuda.set_device(gpu_id)
                gpu_id = (
...
def _wait_and_warmup(server_args, pipe_finish_writer):
    ...
    ctx_sizes = getattr(server_args, "warmup_ctx_sizes", [32, 64, 512, 2048])
    new_tokens_list = [1, 32, 128] if model_info.get("is_generation") else [1]
    repeats = getattr(server_args, "warmup_repeats", 2)
    concurrency = getattr(server_args, "warmup_concurrency", 24)  # number of concurrent requests

    payloads = []
    for _dp in range(getattr(server_args, "dp_size", 1)):
        for _ in range(repeats):
            for L in ctx_sizes:
                for M in new_tokens_list:
                    p = {"sampling_params": {"temperature": 0, "max_new_tokens": M}}
                    if server_args.skip_tokenizer_init:
                        p["input_ids"] = list(range(10, 10 + L))
                    else:
                        p["text"] = ("Lorem ipsum " * ((L // 2) + 1)).strip()
                    payloads.append(p)

    try:
        with requests.Session() as s:
            s.headers.update(headers or {})
            s.mount("http://", HTTPAdapter(pool_connections=concurrency, pool_maxsize=concurrency))
            s.mount("https://", HTTPAdapter(pool_connections=concurrency, pool_maxsize=concurrency))

            with ThreadPoolExecutor(max_workers=concurrency) as ex:
                futs = [ex.submit(s.post, url + request_name, json=p, timeout=600) for p in payloads]
