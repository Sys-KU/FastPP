...
from vllm.distributed import get_tensor_model_parallel_world_size, get_tp_group, get_pp_group
...
class LlamaForCausalLM(nn.Module):
    ...
    def forward(
        ...
        """
        If the current process pipeline (PP) rank is not the last, 
        model_output is intermediate tensors. 
        If it is the last rank, model_output is hidden states.
        """
        model_output = self.model(input_ids, positions, forward_batch, input_embeds, intermediate_tensors)
        ...
        if not get_embedding:
            ...
            return self.pooler(model_output, forward_batch)
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            ...
            #         logger.warning(f"{name} layer was not loaded")
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                ...
                if name in params_dict:
                    # if get_pp_group().is_last_rank:
                    #     logger.warning(f"1, {weight_name}, {name} layer was loaded")
                ...
                if name in params_dict:
                    # if get_pp_group().is_last_rank:
                    #     logger.warning(f"2, {name} layer was loaded")
                    param = params_dict[name]
