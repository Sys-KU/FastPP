...
class SamplingBatchInfo:
    ...
    def copy(self) -> "SamplingBatchInfo":
        # Create a new object via shallow copy
        new_info = SamplingBatchInfo(
            temperatures=self.temperatures.clone() if self.temperatures is not None else None,
            top_ps=self.top_ps.clone() if self.top_ps is not None else None,
            top_ks=self.top_ks.clone() if self.top_ks is not None else None,
            min_ps=self.min_ps.clone() if self.min_ps is not None else None,
            is_all_greedy=self.is_all_greedy,
            need_min_p_sampling=self.need_min_p_sampling,
            vocab_size=self.vocab_size,
            grammars=self.grammars.copy() if self.grammars else None,
            sampling_info_done=self.sampling_info_done,
            logit_bias=self.logit_bias.clone() if self.logit_bias is not None else None,
            vocab_mask=self.vocab_mask.clone() if self.vocab_mask is not None else None,
            apply_mask=self.apply_mask,
            penalizer_orchestrator=self.penalizer_orchestrator if self.penalizer_orchestrator else None,
            linear_penalties=self.linear_penalties.clone() if self.linear_penalties is not None else None,
            scaling_penalties=self.scaling_penalties.clone() if self.scaling_penalties is not None else None,
            device=self.device,
        )
        return new_info
