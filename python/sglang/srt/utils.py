...
class PPCounter:
    def __init__(self, pp_size: int, pp_rank: int, enable_overlap: bool):
        """
        Initialize the counter with the size of the pipeline (pp_size, pp_rank).
        """
        if pp_size <= 0:
        ...
        self.current_idx = -1
        self.process_pp_idx = pp_rank
        self.process_pp_idx_last = pp_rank - 1
    ...
    def __next__(self):
        """
        return the next counter value in a circular manner.
        """
        self.current_idx = (self.current_idx + 1) % self.pp_size
        self.process_pp_idx = (self.process_pp_idx + 1) % self.pp_size
        self.process_pp_idx_last = (self.process_pp_idx_last + 1) % self.pp_size
    ...
    def get_pp(self):
        """
        get the idx for pp's processed batch.
        or
        get the idx for pp's transmitted next_token_ids in the enable overlap.
        """
        return self.process_pp_idx

    def get_pp_last(self):
        """
        get the idx for pp's processed next_token_ids in the enable overlap.
        """
        return self.process_pp_idx_last

    def __repr__(self):
        """
        string representation of the ppcounter.
        """
        return (
            f"ppcounter("
            ...
            f"process_pp_idx={self.process_pp_idx})"
        )
...
def recv_object(dist_group, src: int) -> Any:
    """Receive the input object list from the source rank."""
    """NOTE: `src` is the local rank of the source rank."""

    assert src < dist_group.world_size, f"Invalid src rank ({src})"

    assert src != dist_group.rank_in_group, (
...
def recv_tensor_dict(
    ...
    """
    # Bypass the function if we are using only 1 GPU.
    if not torch.distributed.is_initialized() or dist_group.world_size == 1:
        return None
    ...
    metadata_group = dist_group.cpu_group

    if src is None:
        src = (dist_group.rank_in_group - 1) % dist_group.world_size
    assert src < dist_group.world_size, f"Invalid src rank ({src})"

    recv_metadata_list = recv_object(dist_group, src=src)
...
def send_object(dist_group, obj: Any, dst: int) -> None:
    """Send the input object list to the destination rank."""
    """NOTE: `dst` is the local rank of the destination rank."""

    assert dst < dist_group.world_size, f"Invalid dst rank ({dst})"

    assert dst != dist_group.rank_in_group, (
...
def send_tensor_dict(
    ...
    if dst is None:
        dst = (dist_group.rank_in_group + 1) % dist_group.world_size
    assert dst < dist_group.world_size, f"Invalid dst rank ({dst})"
    ...
    send_object(dist_group, metadata_list, dst=dst)
...
def isend_object(dist_group, obj: Any, dst: int) -> None:
    """Asynchronously send the input object list to the destination rank."""
    """NOTE: `dst` is the local rank of the destination rank."""
    ...
    # Asynchronously send object size
    req_size = torch.distributed.isend(size_tensor,
                                       dst=dist_group.ranks[dst],
                                       group=dist_group.cpu_group)

    # Asynchronously send object
    req_object = torch.distributed.isend(object_tensor,
                                         dst=dist_group.ranks[dst],
                                         group=dist_group.cpu_group)

    # Ensure that send operations are complete before proceeding
    # req_size.wait()
    req_object.wait()
...
def isend_tensor_dict(
    ...
    """Asynchronously send the input tensor dictionary.
    NOTE: `dst` is the local rank of the source rank.
    """
    # Bypass the function if we are using only 1 GPU.
    if not torch.distributed.is_initialized() or dist_group.world_size == 1:
    ...
    # `metadata_list` lives in CPU memory.
    # `send_object_list` has serialization & deserialization, all happening on CPU.
    # Therefore, we can use the CPU group.
    isend_object(dist_group, metadata_list, dst=dst)
    ...
    for tensor in tensor_list:
        ...
        if tensor.is_cpu:
            # Use metadata_group for CPU tensors
            req = torch.distributed.isend(tensor,
                                          dst=dist_group.ranks[dst],
                                          group=metadata_group)
        else:
            # Use group for GPU tensors
    ...
    # Wait for all async send operations to complete
...
def isend_tensor(
    ...
    """
    # Bypass the function if we are using only 1 GPU.
    if not dist.is_initialized() or dist_group.world_size == 1:
        return
...
def recv_tensor(
    ...
    tensor = torch.empty(expected_shape,
                         dtype=expected_dtype,
                         device=dist_group.device)
    if tensor.numel() == 0:
        # Skip broadcasting empty tensors.
        return tensor
...
def broadcast(dist_group, input_: torch.Tensor, src: int = 0):
...
def get_zmq_pub_sub_socket(context: zmq.Context, socket_type: zmq.SocketType, endpoint: str):
    mem = psutil.virtual_memory()
    total_mem = mem.total / 1024**3
    available_mem = mem.available / 1024**3
    if total_mem > 32 and available_mem > 16:
...
def set_gpu_proc_affinity(
    ...
    pp_size_per_node = pp_size // nnodes

    # total physical cores
    total_pcores = psutil.cpu_count(logical=False)
    # physical cores per TP * PP (N.B. more Cores than GPUs on node)
    num_cores_bind = total_pcores // (tp_size_per_node * pp_size_per_node)
