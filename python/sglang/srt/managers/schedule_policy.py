...
class PrefillAdder:
    def __init__(
        ...
        self.tree_cache = tree_cache
        self.running_batchs = running_batchs
        ...
        for running_batch in running_batchs:
            if running_batch is not None:
                ...
                self.rem_total_tokens -= sum(
                    [
                        min(
                            (r.sampling_params.max_new_tokens - r.output_ids_len),
    ...
    def _prefill_one_req(
        ...
        if fetch:
            self.rem_total_tokens -= prefix_len
            self.cur_rem_tokens -= prefix_len
    ...
    def add_one_req_ignore_eos_safe(self, req: Req):
        def add_req_state(r, insert_sort=False):
            new_token_ratio = (
                1.0 if r.sampling_params.ignore_eos else self.new_token_ratio
            )
            # Compute base remaining token count
            base_tokens_left = r.sampling_params.max_new_tokens * new_token_ratio - r.output_ids_len
            pp_safety_margin = 0
            if self.pp_size > 1:
                pp_safety_margin = self.pp_size - 1
            tokens_left = base_tokens_left + pp_safety_margin
            tokens_occupied = len(r.origin_input_ids) + r.output_ids_len
            # if req.enable_overlap and req.pp_is_first_rank:
            #     tokens_left -= 1
            #     tokens_occupied += 1
        ...
        if self.req_states is None:
            self.req_states = []
            add_req_state(req)
            for running_batch in self.running_batchs:
                if running_batch is not None:
                    for r in running_batch.reqs:
                        add_req_state(r)
    ...
    def add_one_req(self, req: Req, fetch_chunked: bool = False):
        if req.sampling_params.ignore_eos and self.tree_cache.disable:
            return self.add_one_req_ignore_eos_safe(req)
